...
class ContrastiveLoss(nn.Module):
    ...
    Args:
        ...
        local_loss (bool): Whether to use local loss for distributed training
        ...
        use_horovod (bool): Whether to use horovod
    """
    def __init__(self, temperature=0.07, use_hard_negatives=False, margin=0.1,
                 local_loss=False, gather_with_grad=False, cache_labels=False,
    ...
    def forward(self, image_features, text_features, logit_scale, output_dict=False):
        ...
        if self.use_hard_negatives:
            loss = self._hard_negative_loss(logits_per_image, logits_per_text, mask)
        else:
            loss = self._standard_contrastive_loss(logits_per_image, logits_per_text, mask)
...
class NTXentLoss(nn.Module):
    ...
    def forward(self, image_features, text_features, logit_scale, output_dict=False):
        ...
        # Apply temperature scaling
        similarity_matrix = logits_per_image / self.temperature
...
class TripletLoss(nn.Module):
    ...
    def forward(self, image_features, text_features, logit_scale, output_dict=False):
        ...
        # Assume diagonal pairs are positive
